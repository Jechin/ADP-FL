...
class BaseFederatedTrainer(object):
    ...
    def train(self, model, data_loader, optimizer, loss_fun):
        ...
        for step, data in enumerate(data_loader):
            ...
            output = model(inp)
            import sys
            self.logging.info(f"{ __file__ } - Line {sys._getframe().f_lineno}")
            sys.exit(0)
            if self.args.data.startswith("prostate"):
            ...
            # 计算更新前后的差异
            param_diffs = {name: (after - before) for (name, before), (_, after) in zip(params_before.items(), params_after.items())}
            print("10")
            # 输出更新差异
            for name, diff in param_diffs.items():
                print(f"Parameter: {name}, Update Difference: {diff}")
